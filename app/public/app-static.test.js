...
test("learning library is reachable from sidebar and renders readonly tabs", () => {
  ...
  assert.match(appSource, /function switchLearningLibraryTab/);
  assert.match(appSource, /data-learning-library-tab-group/);
  assert.match(appSource, /event\.stopPropagation\(\)/);
  assert.match(appSource, /\.learning-help-wrap/);
  ...
  assert.match(stylesSource, /\.learning-capability-summary/);
  ...
  assert.match(stylesSource, /\.learning-library-tab-group \{[\s\S]*cursor: pointer/);
