...
function New-CustomerPackage {
  ...
  New-TextFile -Path (Join-Path $stage "PACKAGE-CONTENTS.txt") -Lines @(
    ...
    "",
    "Version: v$Version",
    "Mode: $(if ($IncludeSkills) { 'Package with skills directory' } else { 'Update package without skills overwrite' })",
    "",
    "Excluded on purpose:",
    "- .env and .env.local",
    "- app/config/deepseek.local.json",
    ...
    "",
    "Skill policy:",
    "$(if ($IncludeSkills) { '- This package includes the skills directory. Use this mode when a release updates skill behavior or learning architecture.' } else { '- This package does not include the skills directory, so it will not overwrite customer skills.' })",
    "",
