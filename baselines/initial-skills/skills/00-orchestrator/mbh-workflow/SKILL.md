---
name: mbh-workflow
description: AI漫剧剧本分镜总控技能。用于用户提供小说、剧本、分镜、反馈或样例时，选择正确链路，并按稳定 skill 驱动架构调度输入整理、剧本生成、剧本评审、分镜生成、学习资料沉淀和技能进化。
---
...
它负责判断用户给的是什么材料、应该走哪条链路、产物应该保存到哪里、是否需要进入学习资料库或技能进化。

当前阶段采用稳定 skill 驱动生成：实时生成链路只读取必要基础 skill 和正式可调用小技能。学习资料库只做证据留痕和沉淀建议，不自动参与本轮生成。
...
- 明确可程序检查的交付约束，例如“分镜台词每句 20 字以内”，必须进入稳定分镜 skill 的提示和生成后硬规则校验；学习资料库记录只能作为后续改 skill 的证据。
